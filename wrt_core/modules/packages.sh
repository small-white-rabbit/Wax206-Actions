...
remove_unwanted_packages() {
    ...
    local packages_broken=(
        "fatresize" "onionshare-cli" "python-platformio" "python-uvicorn"
    )
    ...
    # 移除依赖缺失导致 config-package.in 语法错误的包
    for pkg in "${packages_broken[@]}"; do
        find ./feeds -type d -name "$pkg" -exec \rm -rf {} + 2>/dev/null
    done
